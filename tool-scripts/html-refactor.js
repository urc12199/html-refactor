...
function getDefaultConfig(projectR) {
  ...
  return {
    ...
    groupUtilityClasses: { // New feature configuration
        enabled: false, // Disabled by default
        prefix: 'g-', // Prefix for generated classes
        ignoreClasses: [], // Classes to ignore during grouping
    },
...
async function interactiveConfigSetup(currentProjectRoot, existingConfig = null, nonInteractive = false) {
  ...
  const questions = [
    ...
    // New questions for groupUtilityClasses
    {
        type: 'confirm',
        name: 'groupUtilityClasses.enabled',
        message: 'Enable grouping of utility CSS classes (e.g., from Tailwind) into a single generated class?',
        default: initialValues.groupUtilityClasses ? initialValues.groupUtilityClasses.enabled : false,
    },
    {
        type: 'input',
        name: 'groupUtilityClasses.prefix',
        message: 'Prefix for generated class names (e.g., "g-"):',
        default: initialValues.groupUtilityClasses ? initialValues.groupUtilityClasses.prefix : 'g-',
        when: (answers) => answers['groupUtilityClasses.enabled'],
    },
    {
        type: 'input',
        name: 'groupUtilityClasses.ignoreClasses',
        message: 'Comma-separated list of CSS classes to ignore during grouping (e.g., js-hook,active):',
        default: initialValues.groupUtilityClasses ? initialValues.groupUtilityClasses.ignoreClasses.join(',') : '',
        when: (answers) => answers['groupUtilityClasses.enabled'],
        filter: (input) => input.split(',').map(s => s.trim()).filter(s => s.length > 0),
    }
  ];
...
// --- Utility Class Grouping Logic ---

/**
 * Finds unique combinations of utility classes, replaces them with a generic class,
 * and returns the necessary @apply rules for CSS generation.
 * @param {object} $ - The Cheerio object representing the loaded HTML.
 * @param {object} config - The application configuration object.
 * @returns {{applyRules: string[], modified: boolean}} An object containing the CSS @apply rules and a flag indicating if the HTML was changed.
 */
function groupAndReplaceUtilityClasses($, config) {
    const classMap = new Map();
    const applyRules = [];
    let classCounter = 0;
    let modified = false;
    const prefix = config.groupUtilityClasses.prefix || 'g-';
    const ignoreClasses = new Set(config.groupUtilityClasses.ignoreClasses || []);

    $('[class]').each((index, element) => {
        const $element = $(element);
        const originalClasses = ($element.attr('class') || '').split(/\s+/).filter(c => c);

        // Also ignore any classes that look like they were generated by this script
        const classesToGroup = originalClasses.filter(c => !ignoreClasses.has(c) && !c.startsWith(prefix));

        if (classesToGroup.length === 0) {
            return; // Skip elements with no groupable classes
        }

        const remainingClasses = originalClasses.filter(c => ignoreClasses.has(c));
        const classKey = [...new Set(classesToGroup)].sort().join(' ');

        if (!classKey) return;

        let genericClassName;
        if (classMap.has(classKey)) {
            genericClassName = classMap.get(classKey);
        } else {
            classCounter++;
            genericClassName = `${prefix}${classCounter}`;
            classMap.set(classKey, genericClassName);
            const applyRule = `.${genericClassName} { @apply ${classKey}; }`;
            applyRules.push(applyRule);
        }

        const newClassAttr = [...remainingClasses, genericClassName].join(' ');
        if ($element.attr('class') !== newClassAttr) {
            $element.attr('class', newClassAttr);
            modified = true;
        }
    });

    if (modified) {
        loggerInstance.info(`Grouped ${applyRules.length} unique sets of utility classes into generic classes (e.g., "${prefix}1").`);
    }
...
/**
 * Runs the Tailwind CSS build process on a given set of @apply rules.
 * @param {string[]} applyRules - An array of CSS rules using @apply.
 * @param {object} config - The application configuration object.
 * @returns {Promise<string>} The compiled CSS content.
 */
async function runTailwindBuild(applyRules, config) {
    if (applyRules.length === 0) {
        return '';
    }

    const tailwindConfigPath = path.join(config.projectRoot, 'tailwind.config.js');
    if (!await fs.pathExists(tailwindConfigPath)) {
        loggerInstance.warn('tailwind.config.js not found in project root. Skipping utility class CSS generation.');
        return '';
    }

    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), 'href-build-'));
    const tempInputFile = path.join(tempDir, 'temp-input.css');
    const tempOutputFile = path.join(tempDir, 'temp-output.css');

    try {
        const cssContent = applyRules.join('\n');
        await fs.writeFile(tempInputFile, cssContent);

        const tailwindExecutable = path.join(config.projectRoot, 'node_modules', '.bin', 'tailwindcss');

        if (!await fs.pathExists(tailwindExecutable)) {
            loggerInstance.error('`tailwindcss` executable not found in `node_modules/.bin/`.');
            loggerInstance.error('Please ensure tailwindcss is installed as a dev dependency in your project.');
            return '';
        }

        const command = `"${tailwindExecutable}" -i "${tempInputFile}" -o "${tempOutputFile}"`;
        loggerInstance.info('Running Tailwind CSS build for generated classes...');
        loggerInstance.debug(`Executing command: ${command}`);

        execSync(command, { stdio: 'pipe', cwd: config.projectRoot }); // stdio: 'pipe' to suppress output unless error

        const compiledCss = await fs.readFile(tempOutputFile, 'utf-8');
        loggerInstance.success('Successfully compiled CSS for grouped utility classes.');
        return compiledCss;

    } catch (error) {
        loggerInstance.error('Failed to build CSS with Tailwind CLI.');
        loggerInstance.error('Please ensure tailwindcss is installed in your project (`npm install -D tailwindcss`).');
        loggerInstance.error(`Error details: ${error.stderr ? error.stderr.toString() : error.message}`);
        return ''; // Return empty string on failure
    } finally {
        await fs.remove(tempDir); // Clean up the temporary directory
        loggerInstance.debug(`Cleaned up temporary build directory: ${tempDir}`);
    }
}
...
async function processHtmlFile(htmlFilePath, config, isDryRun) {
  ...
  try {
    ...
    // --- CSS and Utility Class Processing ---

    // 1. Extract CSS from <style> tags and inline style attributes
    let { cssContent: extractedCss, cssModified: styleTagsAndAttrsRemoved } = await extractCssFromHtml($, htmlFilePath, htmlFileRawBaseName);
    let groupedClassesModified = false;

    // 2. Group utility classes if the feature is enabled
    if (config.groupUtilityClasses && config.groupUtilityClasses.enabled) {
        const { applyRules, modified } = groupAndReplaceUtilityClasses($, config);
        groupedClassesModified = modified;

        if (applyRules.length > 0) {
            const compiledUtilityCss = await runTailwindBuild(applyRules, config);
            if (compiledUtilityCss) {
                extractedCss += `\n\n/* --- Compiled Utility Classes --- */\n${compiledUtilityCss}`;
            }
        }
    }

    // 3. Save combined CSS and update HTML with the link
    let newCssLinkAdded = false;
    if (extractedCss.trim()) {
        const cssFileBase = generateCssFileBaseName(htmlFilePath, config);
        const savedCssPath = await saveCssFile(extractedCss, cssFileBase, htmlFilePath, config, isDryRun);
    ...
    const sanitizedHtmlBaseForJs = sanitizeStringForFilename(htmlFileRawBaseName);
    const { jsModifiedInHtml, extractedJsFiles } = await extractJsFromHtml($, htmlFilePath, sanitizedHtmlBaseForJs, config, isDryRun);

    // --- Final Save Condition Check ---
    const effectiveChangeMade = styleTagsAndAttrsRemoved || groupedClassesModified || jsModifiedInHtml || newCssLinkAdded;
...
async function cli() {
  ...
  const argv = await yargs(hideBin(process.argv))
    ...
    .command('$0', 'Refactor HTML files: extract inline styles & scripts, update links.', (yargs) => {
        ...
        // Overrides for groupUtilityClasses
        if (!config.groupUtilityClasses) { // Ensure object exists
            config.groupUtilityClasses = {};
        }
        if (args.groupUtils !== undefined) {
            config.groupUtilityClasses.enabled = args.groupUtils;
        }
        if (args.groupUtilsPrefix) {
            config.groupUtilityClasses.prefix = args.groupUtilsPrefix;
        }
        if (args.groupUtilsIgnore) {
            config.groupUtilityClasses.ignoreClasses = String(args.groupUtilsIgnore).split(',').map(s => s.trim()).filter(s => s);
        }
    ...
    // New options for groupUtilityClasses
    .option('group-utils', {
        type: 'boolean',
        description: 'Enable grouping of utility CSS classes. Overrides config.'
    })
    .option('group-utils-prefix', {
        type: 'string',
        description: 'Prefix for generated class names (e.g., "g-"). Overrides config.'
    })
    .option('group-utils-ignore', {
        type: 'string',
        description: 'Comma-separated CSS classes to ignore during grouping. Overrides config.'
    })
    .option('yes', {
      alias: 'y',
      type: 'boolean',
